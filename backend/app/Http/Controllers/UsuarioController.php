...
class UsuarioController extends Controller
{
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        $vinculos = $this->resolverVinculos(
            $datos
        );
        ...
        $usuario = User::create([
            ...
            'federacion_id' =>
                $vinculos['federacion_id'],
            'sindicato_id' =>
                $vinculos['sindicato_id'],
            'mototaxista_id' => null,
            'pasajero_id' => null,
        ]);
    ...
    public function update(
        ...
        return response()->json([
            'message' =>
                'Usuario administrativo actualizado correctamente.',
            'data' => $usuario->fresh([
                'persona',
                'federacion',
                'sindicato.federacionEntidad',
    ...
    private function validar(
        ...
        return $request->validate([
            ...
            'nickname.min' =>
                'El nickname debe tener al menos 4 caracteres.',
            'nickname.unique' =>
            ...
            'sindicato_id.required' =>
                'El secretario debe estar vinculado a un sindicato.',
            'sindicato_id.exists' =>
                'El sindicato seleccionado no existe.',
        ]);
    }

    private function resolverVinculos(
        array $datos
    ): array {
        if (
            ($datos['role'] ?? null)
        ...
        $sindicato = Sindicato::findOrFail(
            (int) $datos['sindicato_id']
        );
    ...
    private function resolverEmail(
        ?string $email,
        string $nickname
    ): string {
        ...
        return mb_strtolower($nickname)
            . '@motrix.local';
    }

    private function nombrePersona(
        Persona $persona
    ): string {
        return trim(
            $persona->nombre
